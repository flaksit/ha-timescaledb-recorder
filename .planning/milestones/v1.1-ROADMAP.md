...
4. psycopg3 `Jsonb()` wrappers and `TEXT[]` list adaptation produce correct inserts for the `states` hypertable columns
...
2. On startup and on DB-healthy transition, backfill fetches state changes from HA sqlite since `MAX(last_updated)` in `states`, applies the entity filter, and writes in chunks of ≤1000 rows; backfill does not block HA startup
