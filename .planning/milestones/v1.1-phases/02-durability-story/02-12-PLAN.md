...
<task type="auto">
  ...
  <read_first>
    - custom_components/timescaledb_recorder/states_worker.py (entire file)
    - custom_components/timescaledb_recorder/worker.py (StateRow.from_state shape)
  ...
  - `test_slice_to_rows_sorts_by_last_updated` also confirms the `from_state` code path works with MagicMock (any object with the right attributes).
  </action>
  <verify>
...
<task type="auto">
  ...
  <files>tests/test_states_worker.py, tests/test_meta_worker.py, tests/test_backfill.py</files>
  <read_first>
    - custom_components/timescaledb_recorder/meta_worker.py (entire file)
    - custom_components/timescaledb_recorder/const.py (SCD2_* constants)
    - custom_components/timescaledb_recorder/syncer.py (MetadataSyncer — mock target)
...
<task type="auto">
  ...
  <files>tests/test_states_worker.py, tests/test_meta_worker.py, tests/test_backfill.py</files>
  <read_first>
    - custom_components/timescaledb_recorder/backfill.py (entire file)
  ...
  def test_fetch_slice_raw_iterates_per_entity_never_passes_none():
      ...
      with patch(
          "custom_components.timescaledb_recorder.backfill.recorder_history.state_changes_during_period"
      ) as mock_sc:
